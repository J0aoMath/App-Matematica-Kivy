...
class Menu_Inicial(BoxLayout):
    def on_press_operacoes(self):
        print('Operações Básicas')
        self.remove_widget(self)
        #self.add_widget(Principal().menu_operacoes)
    def on_press_equacoes(self):
        print('Equações do primeiro grau')
        #self.remove_widget(self)
        #self.add_widget(Principal().menu_equacoes)
    lb1 = Label(text='[color=ff3333]Exercícios de Matemática[/color]', markup=True)
    lb2 = Label()
    bt1 = Button(text='Operações Basicas', on_press = on_press_operacoes)
    bt2 = Button(text='Expressões do primeiro grau', on_press = on_press_equacoes)
    bt3 = Button(text='Estatísticas')
    bt4 = Button(text='Opções')
    bt5 = Button(text='Instruções')
    def __init__(self, **kwargs):
